...
const props = defineProps({
    buyerUsers: { type: Object, required: true },
...
const rows = computed(() => props.buyerUsers.data ?? [])
...
const apply = () => {
    ...
    router.get(route('admin.users.buyers.index'), query, { preserveState: true, replace: true })
}

const toggle = (item) => router.post(route('admin.users.buyers.toggle-active', item.id))
const unlink = (item) => {
    if (confirm('¿Eliminar solo el vínculo con el comprador? (preferible desactivar)')) {
        ...
        <div class="row page-titles mx-0">
            <div class="col-sm-6 p-md-0">
                <div class="welcome-text">
                    ...
                    <span>Acceso al portal /buyer</span>
                </div>
            </div>
            <div class="col-sm-6 p-md-0 d-flex justify-content-sm-end">
                <Link :href="route('admin.users.buyers.create')" class="btn btn-primary">Nuevo usuario comprador</Link>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-body table-responsive">
                <table class="table mb-0">
                    ...
                    <tbody>
                        <tr v-if="!rows.length">
                            <td colspan="8" class="text-center text-muted">Sin usuarios</td>
                        </tr>
                        <tr v-for="item in rows" :key="item.id">
                            ...
                            <td>{{ item.credit_allowed ? 'Sí' : 'No' }}</td>
                            <td>{{ item.credit_days_default ?? '—' }}</td>
                            <td class="admin-actions-column">
                                <div class="admin-actions">
                                    <Link :href="route('admin.users.buyers.show', item.id)" class="btn btn-info btn-xs shadow">Ver</Link>
                                    <Link :href="route('admin.users.buyers.edit', item.id)" class="btn btn-success btn-xs shadow">Editar</Link>
